...
public class NrfMeshRepository implements MeshProvisioningStatusCallbacks, MeshStatusCallbacks,
    ...
    public NrfMeshRepository(final MeshManagerApi meshManagerApi,
        ...
        if (!alreadyImported) {
            ...
            if (defaultJson != null) {
                ...
                Log.d(TAG, "✅ Default mesh network (_net.jsi25on) imported on first launch");
            } else {
    ...
    private String resolveServerKeyByNodeName(@Nullable final String normalizedNodeName) {
        ...
        final String deviceCode = DeviceCodes.getCode(normalizedNodeName);

        for (final String storedKey : allKeys) {
            final String lowerStored = storedKey.toLowerCase();
            if (extractPureNameFromKey(storedKey).equals(normalizedNodeName)
                    || DeviceCodes.matches(normalizedNodeName, storedKey)
                    || (deviceCode != null && lowerStored.contains(deviceCode.toLowerCase()))) {
    ...
    private void sendNextAutoBind() {
        ...
        if (mAutoBindIndex >= mPendingBindOperations.size()) {
            ...
            if (mAutoSetupCompleteListener != null && completedNode != null) {
                setupAutoPublication(completedNode);
    ...
    private void saveClientElementAddresses(@NonNull final ProvisionedMeshNode node,
        ...
        Log.d(TAG_BIND, "🔥 ==================================================");
        Log.d(TAG_BIND, "🔥 [TEMP_LOG] CLIENT FIRST 5 ELEMENTS FOR KEY: '" + clientKey + "'");
        for (final Element element : sortedElements) {
            ...
            if (hasClientModel) {
                ...
                if (clientIndex < 5) {
                    Log.d(TAG_BIND, "🔥 [TEMP_LOG] Client Element Index [" + clientIndex + "] -> Address: 0x"
    ...
    // =========================================================================
    // AUTO PUBLICATION (Control Node <-> Single Switch)
    // =========================================================================

    private void setupAutoPublication(@NonNull final ProvisionedMeshNode newNode) {
        final String name = newNode.getNodeName();
        if (name == null) return;

        // Check if it's one of the target server devices for auto-link
        boolean isTargetServer = name.toLowerCase().contains("single switch node") ||
                                 name.toLowerCase().contains("singleknob node living area");

        if (!isTargetServer) {
            return;
        }

        Log.d(TAG_BIND, "🔍 Auto-Pub Detected for: '" + name + "'. Notifying SharedViewModel for SVG-based mapping.");

        if (mAutoSetupCompleteListener != null) {
            mHandler.post(() -> mAutoSetupCompleteListener.onAutoSetupComplete(newNode));
        }
    }

    private Element findFirstFreeClientElement(@NonNull final ProvisionedMeshNode node) {
        final List<Element> elements = new ArrayList<>(node.getElements().values());
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            elements.sort((a, b) -> Integer.compare(a.getElementAddress(), b.getElementAddress()));
        }

        // Start from index 1 to skip Config Server on element 0
        for (int i = 1; i < elements.size(); i++) {
            final Element element = elements.get(i);
            final MeshModel model = element.getMeshModels().get(MODEL_GENERIC_ONOFF_CLIENT);
            if (model != null) {
                if (model.getPublicationSettings() == null || model.getPublicationSettings().getPublishAddress() == 0) {
                    return element;
                }
            }
        }
