...
SharedViewModel extends BaseViewModel
    ...
    private static final long PUB_TIMEOUT_MS  = 12_000L;
 // 8 seconds per attempt
    ...
    // Tracks nodes currently undergoing auto-publication to avoid double-triggers
    private final Set<Integer> mNodesInSetup = new HashSet<>();
    ...
    private void onAutoSetupComplete(@NonNull ProvisionedMeshNode node) {
        int nodeAddr = node.getUnicastAddress();
        if (mNodesInSetup.contains(nodeAddr)) {
            Log.d(TAG, "🚫 onAutoSetupComplete: Already in progress for 0x" + String.format("%04X", nodeAddr));
            return;
        }
        
        Log.d(TAG, "🔔 onAutoSetupComplete: node=0x"
                + String.format("%04X", nodeAddr)
                + " name=" + node.getNodeName());
        
        mNodesInSetup.add(nodeAddr);
        new Handler(Looper.getMainLooper())
                .postDelayed(() -> {
                    triggerPublicationSetup(node);
                    // Clear after some time to allow future re-setups if needed
                    new Handler(Looper.getMainLooper()).postDelayed(() -> mNodesInSetup.remove(nodeAddr), 30000);
                }, 500);
    }
    ...
    private void triggerPublicationSetup(@NonNull ProvisionedMeshNode completedNode) {
        ...
        for (String serverKey : provisionedKeys) {
            ...
            // ── 3. SVG Metadata (EID and RID) ──────────────────────────────
            int eid = ClientServerElementStore.getServerSvgElementId(serverKey);
            String ridStr = ClientServerElementStore.getReceiveId(serverKey);
            int rid = -1;
            try {
                if (ridStr != null && !ridStr.trim().isEmpty()) {
                    rid = Integer.parseInt(ridStr.trim());
                }
            } catch (NumberFormatException ignored) {}

            if (eid == -1) {
                Log.w(TAG, "  No EID found for " + serverKey);
                continue;
            }

            // ── Find Control Node Object and list its Client (1001) elements ──
            ProvisionedMeshNode controlNodeObj = null;
            if (allNodes != null) {
                for (ProvisionedMeshNode n : allNodes) {
                    if (n.getUnicastAddress() == controlNodeAddr) {
                        controlNodeObj = n;
                        break;
                    }
                    for (Element el : n.getElements().values()) {
                        if (el.getElementAddress() == controlNodeAddr) {
                            controlNodeObj = n;
                            break;
                        }
                    }
            ...
            List<Element> client1001Elements = new ArrayList<>();
            if (controlNodeObj != null) {
                List<Element> sortedControlElements = new ArrayList<>(controlNodeObj.getElements().values());
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    sortedControlElements.sort((a, b) ->
                            Integer.compare(a.getElementAddress(), b.getElementAddress()));
                }
                for (Element el : sortedControlElements) {
                    if (el.getMeshModels().containsKey(GENERIC_ONOFF_CLIENT)) {
                        client1001Elements.add(el);
                    }
                }
            }

            Log.d(TAG, "🔥 ==================================================");
            Log.d(TAG, "🔥 [TEMP_LOG] CONTROL NODE CLIENT (1001) ELEMENTS (Count=" + client1001Elements.size() + "):");
            if (controlNodeObj != null) {
                Log.d(TAG, "🔥   Control Node Name: " + controlNodeObj.getNodeName() + " | Unicast Address: 0x" + String.format("%04X", controlNodeObj.getUnicastAddress()));
            }
            for (int i = 0; i < client1001Elements.size(); i++) {
                Log.d(TAG, "🔥   Client (1001) Element Index [" + i + "] -> Address: 0x" + String.format("%04X", client1001Elements.get(i).getElementAddress()));
            }
            Log.d(TAG, "🔥 ==================================================");

            // ── 4. Calculate Client Element Address ────────────────────────
            // If Client 1001 models exist, index directly into client1001Elements (0-based)
            int clientElementAddr = -1;
            if (eid >= 0 && eid < client1001Elements.size()) {
                clientElementAddr = client1001Elements.get(eid).getElementAddress();
            } else {
                int storedAddr = ClientServerElementStore.getClientAddress(serverKey, eid);
                if (storedAddr != -1) {
                    clientElementAddr = storedAddr;
                } else {
                    clientElementAddr = controlNodeAddr + eid;
                }
            }

            int feedbackElementAddr = -1;
            int targetRid = (rid != -1) ? rid : eid;
            if (targetRid >= 0 && targetRid < client1001Elements.size()) {
                feedbackElementAddr = client1001Elements.get(targetRid).getElementAddress();
            } else {
                feedbackElementAddr = clientElementAddr;
            }

            Log.d(TAG, "🔥 ==================================================");
            Log.d(TAG, "🔥 [TEMP_LOG] NEWLY ADDED DEVICE PUBLICATION DETAILS:");
            Log.d(TAG, "🔥   Newly Added Server Device Key: " + serverKey + " (Address: 0x" + String.format("%04X", serverAddr) + ")");
            Log.d(TAG, "🔥   Bound to Client Control Node: 0x" + String.format("%04X", controlNodeAddr));
            Log.d(TAG, "🔥   Client Element Target (C->S): Index " + eid + " -> Address: 0x" + String.format("%04X", clientElementAddr));
            Log.d(TAG, "🔥   Feedback Element Target (S->C): Index " + targetRid + " -> Address: 0x" + String.format("%04X", feedbackElementAddr));
            Log.d(TAG, "🔥 ==================================================");

            final int fCToS = clientElementAddr;
            final int fSToC = feedbackElementAddr;
            final int fSvr  = serverAddr;
            final int fApp  = appKeyIndex;

            new Handler(Looper.getMainLooper()).postDelayed(() -> {
                setupPublicationPairDirectional(fCToS, fSToC, fSvr, fApp);
            }, delayOffset);

            delayOffset += 5000; // 5s gap between different publications
            anyScheduled = true;
        }
    ...
    private void schedulePublicationWithRetry(
        ...
        // Guard against duplicate INITIAL triggers
        PublicationAttempt attempt = mPendingPublications.get(key);
        if (attempt != null && !isRetry) {
            Log.d(TAG, "⚠️ schedulePublicationWithRetry: Setup for " + key + " already in progress — ignoring duplicate trigger");
            return;
        }
    ...
    public boolean fullyDeleteNode(@NonNull ProvisionedMeshNode adapterNode) {
        ...
        String svgId = nodeToSvgMap.get(realNode.getUuid());

        if (svgId == null) {
            svgId = prefs.getString("node_svg_" + realNode.getUuid(), null);
        ...
        // ── Step 3: Clear all publication completion flags for this node's address ──
        int deletedAddr = realNode.getUnicastAddress();
        SharedPreferences.Editor editor = prefs.edit();
        for (Map.Entry<String, ?> entry : prefs.getAll().entrySet()) {
            String k = entry.getKey();
            if (k.startsWith("pub_setup_complete_") || k.startsWith("dir_complete_")) {
                if (k.contains("_" + deletedAddr) || k.contains(String.valueOf(deletedAddr))) {
                    editor.remove(k);
                }
            }
        }
    ...
    public void setSelectedDevice(String deviceName) {
        ...
        if (dev == null) dev = DEFAULT_SELECTED_DEVICE;
        selectedDevice.setValue(dev);
        prefs.edit().putString(KEY_SELECTED_DEVICE, dev).apply();
    }
