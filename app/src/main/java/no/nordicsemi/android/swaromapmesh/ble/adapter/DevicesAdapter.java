...
public class DevicesAdapter extends RecyclerView.Adapter<DevicesAdapter.ViewHolder> {
    ...
    final class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageView      rssi;
        TextView       rssiValue;
        ExtendedBluetoothDevice boundDevice;
        ...
        private ViewHolder(final @NonNull DeviceItemBinding binding) {
            ...
            binding.getRoot().setOnClickListener(v -> {
                if (mOnItemClickListener != null) {
                    ...
                    if (pos >= 0 && pos < mDisplayedDevices.size()) {
                        selectedDevice = mDisplayedDevices.get(pos);
                    } else if (boundDevice != null) {
                        selectedDevice = boundDevice;
                    }

                    if (selectedDevice != null) {
                        Log.d(TAG, "Item clicked: " + selectedDevice.getName() + " (" + selectedDevice.getAddress() + ")");
                        mOnItemClickListener.onItemClick(selectedDevice);
                    }
                }
            });
        }
